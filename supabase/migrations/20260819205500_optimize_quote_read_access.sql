...
as $$
  select (select app.can('quotes', 'view'))
    and p_quote_id = any((select app.readable_supply_quote_ids()));
$$;
...
using (
  ...
  and id = any((select app.readable_supply_quote_ids()))
);
...
using (
  (select app.can('quotes', 'view'))
  and quote_id = any((select app.readable_supply_quote_ids()))
);
