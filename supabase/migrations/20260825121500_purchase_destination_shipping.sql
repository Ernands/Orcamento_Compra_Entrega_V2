...
begin
  ...
  select
    coalesce(sum(greatest(
      ...
      + case
          when freight.destination_count > 0 then freight.shipping_amount
          else coalesce(item.shipping_amount, 0)
        end
      + item.other_costs,
      0
    )), 0),
    coalesce(bool_or(
      case
        when freight.destination_count > 0 then freight.shipping_pending
        else item.shipping_type = 'pending'
      end
    ), false)
  ...
  left join lateral (
    select
      ...
      coalesce(sum(coalesce(destination.shipping_amount, 0)), 0)::numeric as shipping_amount,
      coalesce(bool_or(destination.shipping_type = 'pending'), false) as shipping_pending
    from public.supply_quote_item_destinations destination
    where destination.quote_item_id = item.id
  ) freight on true
  ...
  select
    ...
    qi.discount_amount,
    case
      when freight.destination_count = 0 then qi.shipping_type
      when freight.shipping_pending then 'pending'::public.supply_shipping_type
      when freight.shipping_amount = 0 then 'free'::public.supply_shipping_type
      else 'informed'::public.supply_shipping_type
    end,
    case
      when freight.destination_count = 0 then qi.shipping_amount
      else freight.shipping_amount
    end,
    qi.other_costs,
    greatest(
      round(qi.quantity * qi.unit_price, 2) - qi.discount_amount
      + case
          when freight.destination_count > 0 then freight.shipping_amount
          else coalesce(qi.shipping_amount, 0)
        end
      + qi.other_costs,
      0
    ),
  ...
  left join lateral (
    select
      ...
      coalesce(bool_or(destination.shipping_type = 'pending'), false) as shipping_pending
    from public.supply_quote_item_destinations destination
    where destination.quote_item_id = qi.id
  ) freight on true
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'approved_total', v_total,
      'has_pending_shipping', v_pending,
      'uses_freight_destinations', exists (
        select 1
        from public.supply_quote_items item
        join public.supply_quote_item_destinations destination on destination.quote_item_id = item.id
        where item.quote_id = p_quote_id
      )
    ),
    'database'
