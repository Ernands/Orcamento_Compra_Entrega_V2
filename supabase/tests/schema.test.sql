...
select is((select count(*) from public.permissoes), 27::bigint, 'Vinte e sete capacidades foram criadas');
select is((select count(*) from public.perfil_permissoes pp join public.perfis p on p.id = pp.perfil_id where p.chave = 'consultation'), 8::bigint, 'Consulta recebe somente capacidades de leitura');
select is((select count(*) from public.perfil_permissoes pp join public.perfis p on p.id = pp.perfil_id where p.chave = 'administrator'), 27::bigint, 'Administrador recebe todas as capacidades');
